...
mummerplot $RESULTDIR/flye_vs_flye/genome_flye_itself.delta \
-R $LJA \
-Q $FLYE \
-breaklen 1000 \
--filter \
...
--layout \
--fat \
-p $RESULTDIR/flye_vs_flye/mummerplot

#hifiasm
apptainer exec \
--bind /data \
/containers/apptainer/mummer4_gnuplot.sif \
mummerplot $RESULTDIR/hifiasm_vs_hifiasm/genome_hifiasm_itself.delta \
-R $FLYE \
-Q $HIFIASM \
-breaklen 1000 \
--filter \
...
--layout \
--fat \
-p $RESULTDIR/hifiasm_vs_hifiasm/mummerplot

#LJA
apptainer exec \
--bind /data \
/containers/apptainer/mummer4_gnuplot.sif \
mummerplot $RESULTDIR/genome_LJA_itself.delta \
-R $HIFIASM \
-Q $LJA \
-breaklen 1000 \
--filter \
...
--layout \
--fat \
-p $RESULTDIR/lja_vs_lja/mummerplot
